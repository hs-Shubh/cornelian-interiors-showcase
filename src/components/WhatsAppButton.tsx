...
// Prefer an env override so the number/message can change without a code edit.
const envUrl =
  typeof import.meta.env?.VITE_WHATSAPP_URL === "string"
    ? import.meta.env.VITE_WHATSAPP_URL.trim()
    : "";
const whatsappUrl =
  envUrl || `https://wa.me/${DEFAULT_NUMBER}?text=${encodeURIComponent(PREFILL)}`;

/** Floating click-to-chat WhatsApp button (bottom-right, above the grain layer). */
export function WhatsAppButton() {
  return (
    <a
      ...
      className="fixed bottom-5 right-5 z-[70] flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-[0_8px_24px_rgba(0,0,0,0.35)] transition-transform duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-white/60"
    >
